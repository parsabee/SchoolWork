// OSX compilation:
//    g++ -std=c++11 -I/opt/X11/include -o myfilter2 tbb_myfilter2.cpp -L/opt/X11/lib -lX11 -ljpeg -ltbb
...
main(int argv, char *argc[])
{
	...
	if(numLines !=20){
			cerr << "textfile has to contain exactly 20 paths to jpg images!" << endl;
			return 0;
		}

	cout << "\nreading the list in parallel, processing every image sequentially\n" << endl;
	...
	tbb::parallel_for(size_t(0), size_t(20), [&](size_t a){
		...
		cout << "\nsequential writing time for "<< inputfile[a] <<": "<< chrono:: duration_cast<chrono::microseconds>(duration).count()<<" microseconds\n\n";
