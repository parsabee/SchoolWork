...
should contain three different enum cases: `swimming`, `cycling`, and `running`.

The only additional code necessary in the `Sport` enum is the adoption of a protocol from the Swift standard library
that allows for it to be easily printed to the console output. Modify your `Sport` enum implementation to conform to
the `CustomStringConvertible` protocol and implement the single requirement defined in the `CustomStringConvertible`
protocol:

`public var description: String { get }`

The `swimming` case should return the `String` "swimming", the `cycling` case should return the `String` "cycling", and
