...
static int n = 10;/*number of rows and columns*/
...
int main(int argc, char *argv[]){
	...
	cout << "\nsequential writing time : " << chrono:: duration_cast<chrono::milliseconds>(duration).count() << " milliseconds\n\n";
	...
	*/
	FILE *outputArray = fopen("serial_outputArray.txt", "w");

	for(i=(n+2)/2; i<(n+2); ++i){
		for(j=(n+2)/2; j<(n+2); ++j)
			fprintf(outputArray, "%0.2f	", _output_temp[i][j]);
		fprintf(outputArray, "\n");
	}
